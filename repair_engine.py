...
class AIAgentRepairTool:
    ...
    AGENT_CONFIGS = {
        ...
        "openclaw": {
            ...
            "paths": {
                ...
                "mac": ["~/.openclaw", "~/Library/Application Support/OpenClaw"],
    ...
    def __init__(self, backup_dir: Optional[str] = None):
        ...
        logger.info(f"修复工具初始化完成 - 系统: {platform.system()}, 备份目录: {self.backup_dir}")
    ...
    def _find_agent_path(self, agent_id: str) -> Optional[Path]:
        ...
        system_key = "win" if self.system == "windows" else ("mac" if self.system == "darwin" else "linux")
    ...
    def scan_agent(self, agent_id: str) -> AgentInfo:
        ...
        # 获取版本信息
        version = None
        version_file = install_path / "version" if (install_path / "version").exists() else None
        if not version_file:
            version_file = install_path / "package.json" if (install_path / "package.json").exists() else None
    ...
    def create_backup(self, agent_id: str, backup_name: Optional[str] = None) -> Tuple[bool, str]:
        """
        为指定Agent创建备份
        ...
        try:
            ...
            if agent_info.config_path and agent_info.config_path.exists():
                shutil.copy2(agent_info.config_path, backup_path / agent_info.config_path.name)
            
            # 复制整个配置目录
            shutil.copytree(
                agent_info.install_path,
                backup_path / "full_backup",
                ignore=shutil.ignore_patterns('cache', 'Cache', 'temp', 'Temp', '*.log'),
                dirs_exist_ok=True
            )
    ...
    def restore_backup(self, backup_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if current_info.status != AgentStatus.NOT_INSTALLED:
                self.create_backup(agent_id, f"{agent_id}_pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            ...
            if full_backup_path.exists():
                ...
                # 复制备份到原位置
                shutil.copytree(full_backup_path, original_path, dirs_exist_ok=True)
            ...
            if backup_path.is_dir():
                ...
                if metadata_path.exists():
                    try:
                        ...
                        logger.warning(f"读取备份元数据失败: {backup_path.name}, {e}")
    ...
    def repair_config(self, agent_id: str) -> RepairResult:
        ...
        try:
            ...
            return RepairResult(
                ...
                repair_type=RepairType.CONFIG,
                success=True,
                message="; ".join(fixed_issues) if fixed_issues else "配置文件正常",
                timestamp=datetime.now().isoformat(),
        ...
        try:
            for cache_dir_name in config["cache_dirs"]:
                ...
                if cache_path.exists():
                    ...
                    if freed > 0:
                        cleaned_dirs.append(f"{cache_dir_name} ({freed:.1f}MB)")
                        freed_space += freed
            ...
            return RepairResult(
                ...
                repair_type=RepairType.CACHE,
                success=True,
                message=f"清理完成，释放 {freed_space:.1f}MB" if freed_space > 0 else "缓存已清理",
                timestamp=datetime.now().isoformat(),
    ...
    def repair_plugins(self, agent_id: str) -> RepairResult:
        ...
        try:
            ...
            return RepairResult(
                ...
                repair_type=RepairType.PLUGINS,
                success=True,
                message=f"移除 {len(removed_plugins)} 个损坏插件" if removed_plugins else "插件检查完成",
                timestamp=datetime.now().isoformat(),
    ...
    def full_repair(self, agent_id: str, repair_types: List[RepairType] = None) -> List[RepairResult]:
        """
        执行完整修复流程
        ...
        if repair_types is None:
            repair_types = [RepairType.CONFIG, RepairType.CACHE, RepairType.PLUGINS]
...
def main():
    ...
    parser.add_argument('--scan', action='store_true', help='扫描所有Agent状态')
