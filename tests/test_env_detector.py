...
class TestEnvironmentInfo(unittest.TestCase):
    ...
    def test_is_cross_environment(self):
        """测试跨环境判断"""
        info = EnvironmentInfo(can_access_host=True, target_system="windows")
    ...
    def test_get_icon(self):
        """测试图标获取"""
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.NATIVE).get_icon(), "🖥️"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.WSL).get_icon(), "🐧"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.WSL2).get_icon(), "🪟"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.TERMUX).get_icon(), "📱"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.DOCKER).get_icon(), "🐳"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.VM).get_icon(), "🖧"
        )
        self.assertEqual(
            EnvironmentInfo(type=EnvironmentType.UNKNOWN).get_icon(), "❓"
        )

    def test_get_summary(self):
        """测试摘要获取"""
        info = EnvironmentInfo(
            type=EnvironmentType.WSL2, type_display="WSL2 (Ubuntu)"
        )
...
class TestEnvironmentDetector(unittest.TestCase):
    ...
    def test_detect_os_type_windows(self, mock_release, mock_system):
        ...
        with patch.object(
            detector,
            "_run_cmd",
            return_value=(0, "[Version 10.0.19044.1234]", ""),
        ):
            detector._detect_os_type()
    ...
    def test_detect_wsl2(self, mock_run, mock_exists):
        ...
        mock_run.return_value = MagicMock(
            returncode=0, stdout="5.15.90.1-microsoft-standard-WSL2", stderr=""
        )
    ...
    def test_detect_wsl1(self, mock_run, mock_exists):
        ...
        mock_run.return_value = MagicMock(
            returncode=0, stdout="4.4.0-19041-Microsoft", stderr=""
        )
    ...
    def test_is_cross_environment(self):
        ...
        detector = EnvironmentDetector.__new__(EnvironmentDetector)
        detector.info = EnvironmentInfo(
            can_access_host=True, target_system="windows"
        )
    ...
    def test_validate_agent_path_not_exists(self):
        ...
        with patch("core.env_detector.Path.exists", return_value=False):
            valid, msg = detector.validate_agent_path(
            ...
            with patch("core.env_detector.Path.exists", return_value=True):
                with patch("core.env_detector.re.search") as mock_search:
                    mock_search.return_value = MagicMock(
                        group=lambda x: "Ubuntu 22.04.3 LTS"
                    )
                    detector._detect_os_type()
    ...
    def test_multiple_virtualization_markers(self):
        ...
        with patch("core.env_detector.Path.exists") as mock_exists:
            ...
            def side_effect(self=None):
                ...
                return any(
                    marker in path_str
                    for marker in ["WSLInterop", ".dockerenv", ".containerenv"]
                )
            ...
            with patch("core.env_detector.subprocess.run") as mock_run:
                ...
                self.assertIn(
                    detector.info.type,
                    [EnvironmentType.WSL, EnvironmentType.WSL2],
                )
