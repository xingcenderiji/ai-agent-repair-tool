...
def _cleanup_backups():
    ...
    if backup_dir.exists():
        for item in backup_dir.iterdir():
            if item.name.startswith("testagent_") or item.name.startswith(
                "worker_"
            ):
                try:
                    # 先修复目录本身权限
...
class TestLargeDirectory(TestCase):
    ...
    def test_many_files_in_cache(self):
        ...
        self.assertEqual(issues, [])
        self.assertLess(
            elapsed, 30, f"check_cache took {elapsed:.1f}s, too slow"
        )
    ...
    def test_fix_while_files_change(self):
        ...
        def modifier():
            while not stop_event.is_set():
                try:
                    (
                        cache / f"concurrent_{int(time.time()*1000)}.txt"
                    ).write_text("new")
                except OSError:
                    pass
                time.sleep(0.01)
...
class TestConcurrentSafety(TestCase):
    ...
    def test_concurrent_fix_different_agents(self):
        ...
        AGENT_PATHS["worker_a"] = {
            "name": "WorkerA",
            "paths": {"win": [], "mac": [], "linux": []},
        }
        AGENT_PATHS["worker_b"] = {
            "name": "WorkerB",
            "paths": {"win": [], "mac": [], "linux": []},
        }
        AGENT_PATHS["worker_c"] = {
            "name": "WorkerC",
            "paths": {"win": [], "mac": [], "linux": []},
        }
