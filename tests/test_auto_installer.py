...
from core.auto_installer import (
    AutoInstaller, DetectedFile, FileTypeDetector, FileType,
    InstallStatus, auto_installer
)
...
class TestFileType(unittest.TestCase):
    ...
    def test_file_type_values(self):
        ...
        self.assertEqual(FileType.INSTALLER_APPIMAGE.value, "installer_appimage")
...
class TestDetectedFile(unittest.TestCase):
    ...
    def test_get_icon(self):
        """测试获取图标"""
        file_info = DetectedFile(
            id="file-1", path="/test.exe", filename="test.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0
        )
...
class TestFileTypeDetector(unittest.TestCase):
    ...
    def test_detect_windows_installers(self):
        """测试Windows安装程序检测"""
        self.assertEqual(FileTypeDetector.detect(Path("app.exe")), FileType.INSTALLER_EXE)
        self.assertEqual(FileTypeDetector.detect(Path("app.msi")), FileType.INSTALLER_MSI)
        self.assertEqual(FileTypeDetector.detect(Path("app.msix")), FileType.INSTALLER_MSI)
        self.assertEqual(FileTypeDetector.detect(Path("app.appx")), FileType.INSTALLER_MSI)
    
    def test_detect_macos_installers(self):
        """测试macOS安装程序检测"""
        self.assertEqual(FileTypeDetector.detect(Path("app.pkg")), FileType.INSTALLER_PKG)
        self.assertEqual(FileTypeDetector.detect(Path("app.dmg")), FileType.INSTALLER_DMG)
    
    def test_detect_linux_installers(self):
        """测试Linux安装程序检测"""
        self.assertEqual(FileTypeDetector.detect(Path("app.deb")), FileType.INSTALLER_DEB)
        self.assertEqual(FileTypeDetector.detect(Path("app.rpm")), FileType.INSTALLER_RPM)
        self.assertEqual(FileTypeDetector.detect(Path("app.AppImage")), FileType.INSTALLER_APPIMAGE)
        self.assertEqual(FileTypeDetector.detect(Path("app.appimage")), FileType.INSTALLER_APPIMAGE)
    
    def test_detect_archives(self):
        """测试压缩包检测"""
        self.assertEqual(FileTypeDetector.detect(Path("app.zip")), FileType.ARCHIVE_ZIP)
        self.assertEqual(FileTypeDetector.detect(Path("app.tar")), FileType.ARCHIVE_TAR)
        self.assertEqual(FileTypeDetector.detect(Path("app.gz")), FileType.ARCHIVE_TAR_GZ)
        self.assertEqual(FileTypeDetector.detect(Path("app.tgz")), FileType.ARCHIVE_TAR_GZ)
        self.assertEqual(FileTypeDetector.detect(Path("app.bz2")), FileType.ARCHIVE_TAR)
        self.assertEqual(FileTypeDetector.detect(Path("app.7z")), FileType.ARCHIVE_7Z)
        self.assertEqual(FileTypeDetector.detect(Path("app.rar")), FileType.ARCHIVE_ZIP)
    
    def test_detect_python_packages(self):
        """测试Python包检测"""
        self.assertEqual(FileTypeDetector.detect(Path("app.whl")), FileType.PYTHON_WHEEL)
        self.assertEqual(FileTypeDetector.detect(Path("app.egg")), FileType.PYTHON_EGG)
    
    def test_detect_unknown(self):
        """测试未知文件类型"""
        self.assertEqual(FileTypeDetector.detect(Path("app.txt")), FileType.UNKNOWN)
        self.assertEqual(FileTypeDetector.detect(Path("app.pdf")), FileType.UNKNOWN)
    ...
    def test_is_installer(self):
        ...
        self.assertTrue(FileTypeDetector.is_installer(FileType.INSTALLER_APPIMAGE))
...
class TestAutoInstaller(unittest.TestCase):
    ...
    def test_format_size(self):
        ...
        self.assertEqual(self.installer._format_size(1024*1024*1024), "1.00 GB")
    ...
    def test_identify_target_cursor(self):
        """测试识别Cursor目标"""
        file_info = DetectedFile(
            id="file-1", path="/test.exe", filename="cursor-setup.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_identify_target_claude(self):
        """测试识别Claude目标"""
        file_info = DetectedFile(
            id="file-1", path="/test.pkg", filename="claude-code.pkg",
            file_type=FileType.INSTALLER_PKG, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_identify_target_vscode(self):
        """测试识别VS Code目标"""
        file_info = DetectedFile(
            id="file-1", path="/test.deb", filename="vscode-amd64.deb",
            file_type=FileType.INSTALLER_DEB, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_identify_target_generic_installer(self):
        """测试识别通用安装包"""
        file_info = DetectedFile(
            id="file-1", path="/test.exe", filename="unknown-tool.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_identify_target_archive(self):
        """测试识别压缩包"""
        file_info = DetectedFile(
            id="file-1", path="/test.zip", filename="tools.zip",
            file_type=FileType.ARCHIVE_ZIP, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_prepare_install_command_deb(self):
        """测试准备DEB安装命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.deb", filename="test.deb",
            file_type=FileType.INSTALLER_DEB, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_prepare_install_command_rpm(self):
        """测试准备RPM安装命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.rpm", filename="test.rpm",
            file_type=FileType.INSTALLER_RPM, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_prepare_install_command_appimage(self):
        """测试准备AppImage安装命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.AppImage", filename="test.AppImage",
            file_type=FileType.INSTALLER_APPIMAGE, size_bytes=0, size_display="0 B", modified_time=0,
    ...
    def test_prepare_install_command_zip(self):
        """测试准备ZIP解压命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.zip", filename="test.zip",
            file_type=FileType.ARCHIVE_ZIP, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_prepare_install_command_tar_gz(self):
        """测试准备TAR.GZ解压命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.tar.gz", filename="test.tar.gz",
            file_type=FileType.ARCHIVE_TAR_GZ, size_bytes=0, size_display="0 B", modified_time=0
        )
        ...
        self.assertEqual(file_info.install_args, ["-xzf", "/test.tar.gz", "-C", expected_dir])
    
    def test_prepare_install_command_wheel(self):
        """测试准备Python Wheel安装命令"""
        file_info = DetectedFile(
            id="file-1", path="/test.whl", filename="test.whl",
            file_type=FileType.PYTHON_WHEEL, size_bytes=0, size_display="0 B", modified_time=0
        )
    ...
    def test_clear_completed(self):
        ...
        # 添加不同状态的文件
        file1 = DetectedFile(
            id="file-1", path="/test1.exe", filename="test1.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0,
            status=InstallStatus.SUCCESS
        )
        file2 = DetectedFile(
            id="file-2", path="/test2.exe", filename="test2.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0,
            status=InstallStatus.SKIPPED
        )
        file3 = DetectedFile(
            id="file-3", path="/test3.exe", filename="test3.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0,
...
class TestAutoInstallerInstall(unittest.TestCase):
    ...
    def test_install_appimage(self, mock_run):
        ...
        file_info = DetectedFile(
            id="file-1", path=str(test_file), filename="test.AppImage",
            file_type=FileType.INSTALLER_APPIMAGE, size_bytes=0, size_display="0 B", modified_time=0
        )
        self.installer.detected_files["file-1"] = file_info
    ...
    def test_install_archive_zip(self, mock_run):
        ...
        file_info = DetectedFile(
            id="file-1", path=str(test_file), filename="test.zip",
            file_type=FileType.ARCHIVE_ZIP, size_bytes=0, size_display="0 B", modified_time=0
        )
        self.installer.detected_files["file-1"] = file_info
    ...
    def test_install_unsupported_type(self, mock_run):
        ...
        file_info = DetectedFile(
            id="file-1", path=str(test_file), filename="test.exe",
            file_type=FileType.INSTALLER_EXE, size_bytes=0, size_display="0 B", modified_time=0
        )
        self.installer.detected_files["file-1"] = file_info
