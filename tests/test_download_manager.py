...
class TestDownloadItem(unittest.TestCase):
    ...
    def test_download_item_defaults(self):
        """测试下载项默认值"""
        item = DownloadItem(
            id="test-1", name="test.zip", url="http://example.com/test.zip"
        )
...
class TestDownloadResult(unittest.TestCase):
    ...
    def test_download_result_success(self):
        ...
        result = DownloadResult(
            success=True, file_path="/downloads/file.zip", message="下载完成"
        )
    ...
    def test_download_result_failure(self):
        """测试失败结果"""
        result = DownloadResult(success=False, error="Network timeout")
...
class TestDownloadManager(unittest.TestCase):
    ...
    def test_get_download_links(self):
        ...
        self.manager.add_download(
            "item-1", "file1.zip", "http://a.com/1.zip", 1024
        )
        self.manager.add_download(
            "item-2", "file2.zip", "http://a.com/2.zip", 2048
        )
    ...
    def test_download_file_success(self, mock_request, mock_urlopen):
        ...
        item = DownloadItem(
            id="item-1", name="test.zip", url="http://example.com/test.zip"
        )
    ...
    def test_get_status_single(self):
        """测试获取单个下载状态"""
        self.manager.add_download(
            "item-1", "test.zip", "http://a.com/test.zip", 1024
        )
    ...
    def test_format_size(self):
        ...
        self.assertEqual(
            DownloadManager._format_size(1024 * 1024 * 1024), "1.00 GB"
        )
        self.assertEqual(
            DownloadManager._format_size(1024 * 1024 * 1024 * 2), "2.00 GB"
        )
    ...
    def test_format_speed(self):
        ...
        self.assertEqual(
            DownloadManager._format_speed(1024 * 1024), "1.0 MB/s"
        )

    def test_clear_completed(self):
        """测试清除已完成项"""
        # 添加不同状态的项
        item1 = self.manager.add_download(
            "item-1", "file1.zip", "http://a.com/1.zip"
        )
        item1.status = DownloadStatus.SUCCESS

        item2 = self.manager.add_download(
            "item-2", "file2.zip", "http://a.com/2.zip"
        )
        item2.status = DownloadStatus.FAILED

        item3 = self.manager.add_download(
            "item-3", "file3.zip", "http://a.com/3.zip"
        )
        item3.status = DownloadStatus.CANCELLED

        item4 = self.manager.add_download(
            "item-4", "file4.zip", "http://a.com/4.zip"
        )
...
class TestDownloadManagerEdgeCases(unittest.TestCase):
    ...
    def test_very_small_file(self):
        """测试极小文件"""
        item = self.manager.add_download(
            "item-1", "tiny.txt", "http://a.com/tiny.txt", size_bytes=1
        )
