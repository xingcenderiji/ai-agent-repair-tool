...
class AuditLogger:
    ...
    def log_operation(
        ...
        op_id = (
            f"{self.current_session.session_id}_{self._operation_counter:04d}"
        )
    ...
    def _append_operation_log(self, record: OperationRecord):
        ...
        session_file = (
            self.log_dir / f"session_{self.current_session.session_id}.jsonl"
        )
        try:
    ...
    def end_session(self, summary: Optional[Dict] = None):
        ...
        self.current_session.summary = {
            ...
            "success_count": sum(
                1 for op in ops if op.status == OperationStatus.SUCCESS.value
            ),
            "failed_count": sum(
                1 for op in ops if op.status == OperationStatus.FAILED.value
            ),
            "blocked_count": sum(
                1 for op in ops if op.status == OperationStatus.BLOCKED.value
            ),
            "warning_count": sum(
                1 for op in ops if op.status == OperationStatus.WARNING.value
            ),
            **(summary or {}),
        }
        ...
        # 保存完整会话记录
        session_file = (
            self.log_dir / f"session_{self.current_session.session_id}.json"
        )
        try:
    ...
    def query_operations(
        ...
        for session_file in sorted(
            self.log_dir.glob("session_*.json"), reverse=True
        ):
            try:
                with open(session_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
    ...
    def analyze_failure_root_cause(self, session_id: str) -> Optional[Dict]:
        ...
        blocked_ops = [
            op
            for op in session.operations
            if op.status == OperationStatus.BLOCKED.value
        ]

        if not failed_ops and not blocked_ops:
            return {
                "status": "no_failures",
                "message": "会话中没有失败或拦截的操作",
            }
        ...
        # 生成建议
        if any(
            op.operation == OperationType.CONFIG_FIX.value for op in failed_ops
        ):
            analysis["recommendations"].append(
                "配置文件修复失败，建议检查文件权限或磁盘空间"
            )

        if any(
            op.operation == OperationType.BACKUP.value for op in failed_ops
        ):
            analysis["recommendations"].append(
                "备份操作失败，建议检查备份目录权限和磁盘空间"
            )

        if blocked_ops:
            analysis["recommendations"].append(
                "存在安全拦截操作，建议检查配置是否包含危险路径或操作"
            )
    ...
    def generate_audit_report(
        ...
        for op in operations:
            # 按类型统计
            stats["by_type"][op.operation] = (
                stats["by_type"].get(op.operation, 0) + 1
            )
            # 按状态统计
            stats["by_status"][op.status] = (
                stats["by_status"].get(op.status, 0) + 1
            )
            # 按Agent统计
            if op.agent_id:
                stats["by_agent"][op.agent_id] = (
                    stats["by_agent"].get(op.agent_id, 0) + 1
                )
