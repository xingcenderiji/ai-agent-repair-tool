...
class EnvironmentDetector:
    ...
    def _validate_paths(self):
        ...
        if self.info.type == EnvironmentType.WSL2:
            ...
            if wsl_home:
                ...
                self.info.config_base = "/mnt/c/Users/" + os.environ.get(
                    "USER", "user"
                )
    ...
    def is_cross_environment(self) -> bool:
        """是否跨环境操作"""
        return (
            self.info.can_access_host and self.info.target_system is not None
        )

    def validate_agent_path(
        ...
        if self.info.type == EnvironmentType.WSL2:
            if str(path_obj).replace("\\", "/").startswith("/mnt/"):
        ...
        if self.info.type == EnvironmentType.TERMUX:
            ...
            if termux_prefix and path_str.startswith(
                termux_prefix.replace("\\", "/")
            ):
