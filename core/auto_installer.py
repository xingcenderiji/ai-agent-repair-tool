...
class DetectedFile:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "detected_at": datetime.fromtimestamp(self.modified_time).isoformat(),
        }
...
class FileTypeDetector:
    ...
    def is_installer(cls, file_type: FileType) -> bool:
        """是否是安装程序"""
        return file_type in [
            FileType.INSTALLER_EXE, FileType.INSTALLER_MSI,
            FileType.INSTALLER_PKG, FileType.INSTALLER_DMG,
            FileType.INSTALLER_DEB, FileType.INSTALLER_RPM,
            FileType.INSTALLER_APPIMAGE,
        ]
    ...
    def is_archive(cls, file_type: FileType) -> bool:
        """是否是压缩包"""
        return file_type in [
            FileType.ARCHIVE_ZIP, FileType.ARCHIVE_TAR,
            FileType.ARCHIVE_TAR_GZ, FileType.ARCHIVE_7Z,
        ]
...
class AutoInstaller:
        ...
        for path_template in self.DOWNLOAD_PATHS:
            ...
            try:
                for file_path in path.iterdir():
                    ...
                    # 计算文件ID（基于路径和修改时间）
                    stat = file_path.stat()
                    file_id = hashlib.md5(f"{file_path}:{stat.st_mtime}".encode()).hexdigest()[:12]
        ...
        for agent_id, patterns in self.AGENT_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, filename_lower, re.IGNORECASE):
                    ...
                    # 设置描述
                    file_info.description = f"适用于 {agent_id.title()} 的安装包"
                    break
    ...
    def _prepare_install_command(self, file_info: DetectedFile):
        ...
        if file_info.file_type == FileType.INSTALLER_EXE:
            ...
            extract_dir = Path(file_path).parent / Path(file_path).stem.replace(".tar.gz", "")
            file_info.install_command = "tar"
            file_info.install_args = ["-xzf", file_path, "-C", str(extract_dir)]
        
        elif file_info.file_type == FileType.PYTHON_WHEEL:
            file_info.install_command = "pip"
            file_info.install_args = ["install", file_path]
    
    def install_file(self, file_id: str, auto: bool = True) -> Tuple[bool, str]:
        """执行安装"""
        with self._lock:
        ...
        try:
            ...
            elif file_info.file_type in [FileType.ARCHIVE_ZIP, FileType.ARCHIVE_TAR_GZ]:
                # 解压
                self._install_archive(file_info)
            elif file_info.file_type == FileType.INSTALLER_EXE and sys.platform != 'win32':
                # wine 安装
                self._install_with_wine(file_info)
            else:
                # 其他类型，记录日志但不实际安装
                file_info.install_log.append(f"检测到安装包: {file_info.filename}")
                file_info.install_log.append(f"类型: {file_info.file_type.value}")
            ...
            file_info.install_log.append(f"{datetime.now().isoformat()} 安装完成")
            self._notify(file_info)
            return True, "安装成功"
            ...
            file_info.install_log.append(f"{datetime.now().isoformat()} 安装失败: {e}")
            self._notify(file_info)
            return False, str(e)
    
    def _install_appimage(self, file_info: DetectedFile):
        """安装AppImage"""
        file_info.install_log.append(f"{datetime.now().isoformat()} 添加执行权限...")
        file_info.progress = 20
        self._notify(file_info)
        
        # 添加执行权限
        result = subprocess.run(
            ["chmod", "+x", file_info.path],
            capture_output=True, text=True
        )
        if result.returncode != 0:
            raise Exception(f"添加执行权限失败: {result.stderr}")
        
        file_info.install_log.append(f"{datetime.now().isoformat()} AppImage已准备就绪")
    ...
    def _install_archive(self, file_info: DetectedFile):
        """解压压缩包"""
        file_info.install_log.append(f"{datetime.now().isoformat()} 开始解压...")
    ...
    def _install_with_wine(self, file_info: DetectedFile):
        """用Wine安装"""
        file_info.install_log.append(f"{datetime.now().isoformat()} 使用Wine安装...")
    ...
    def clear_completed(self):
        ...
        with self._lock:
            completed = [
                fid for fid, f in self.detected_files.items()
