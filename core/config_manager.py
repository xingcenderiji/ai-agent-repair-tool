...
class ConfigManager:
    ...
    def __init__(self, local_config_dir: Optional[Path] = None):
        self.local_config_dir = (
            local_config_dir or Path(__file__).parent.parent / "configs"
        )
    ...
    def _is_cache_valid(
        self, cache_path: Path, max_age_hours: int = 24
    ) -> bool:
        """检查缓存是否有效"""
    ...
    def get_config(self, agent_id: str, agent_path: Path) -> AgentConfig:
        ...
        config = AgentConfig(
            ...
            version=version,
            min_supported_version=config_data.get(
                "min_supported_version", "1.0.0"
            ),
            config_files=config_data.get(
                "config_files", ["settings.json", "config.json"]
            ),
            cache_dirs=config_data.get(
    ...
    def match_version(
        self, detected_version: str, config_version: str
    ) -> bool:
        """
        版本匹配
