...
class ConfigScanner:
    ...
    def find_agent_path(self, agent_id: str, paths_config: List[str]) -> Optional[Path]:
        """查找Agent安装路径"""
            ...
            for name in possible_names:
                if name in data:
                    ...
                    if 'key' in key.lower() and isinstance(value, str):
                        preview[key] = value[:4] + "****" if len(value) > 4 else "****"
                    else:
                        preview[key] = value
                    break
        ...
        try:
            ...
            for cf in config_files:
                if cf.exists():
                    ...
                    if cf.name == "settings.json" and info.is_valid and info.content_preview:
                        self._extract_cursor_plugins(result, info.content_preview)
    ...
    def _extract_cursor_plugins(self, result: AgentConfigScan, settings: Dict):
        ...
        # 内置功能作为技能
        builtin_features = [
            "cursor.composer", "cursor.tab", "cursor.cmdK", 
            "cursor.predictions", "cursor.autoImport"
        ]
        for feature in builtin_features:
            enabled = settings.get(feature, True)
            result.skills.append(SkillInfo(
                name=feature,
                enabled=enabled,
                category="core"
            ))
    
    def _scan_extensions_directory(self, result: AgentConfigScan, ext_path: Path):
        """扫描扩展目录"""
        try:
        ...
        result.enabled_plugins = sum(1 for p in result.external_plugins if p.enabled)
        ...
        try:
            ...
            for name, config in servers.items():
                server = ServerInfo(
                    ...
                    config={k: v for k, v in config.items() if k not in ['url', 'enabled']}
                )
    ...
    def scan_claude_config(self, agent_path: Path) -> AgentConfigScan:
        ...
        try:
            ...
            if settings_json.exists():
                try:
                    ...
                    for plugin_id, enabled in enabled_plugins.items():
                        ...
                        result.external_plugins.append(PluginInfo(
                            name=plugin_name,
                            version=None,
                ...
                try:
                    ...
                    for name, config in mcp_servers.items():
                        ...
                        server = ServerInfo(
                            ...
                            type="mcp",
                            enabled=config.get("enabled", True),
                            config={k: v for k, v in config.items() if k not in ['enabled']}
                        )
                        result.mcp_servers.append(server)
                        result.servers.append(server)
                except Exception:
            ...
            if skills_dir.exists():
                self._scan_claude_commands(result, skills_dir, category="skill")
                                ...
                                for manifest_name in ["plugin.json", "package.json"]:
                                    ...
                                    if manifest.exists():
                                        try:
                                            ...
                                            existing = {p.name for p in result.external_plugins}
    ...
    def _scan_claude_commands(self, result: AgentConfigScan, commands_dir: Path, category: str = "command"):
    ...
    def scan_vscode_based_config(self, agent_id: str, agent_name: str, agent_icon: str,
                                  agent_path: Path) -> AgentConfigScan:
        ...
        try:
            ...
            for cf in config_files:
                if cf.exists():
                    ...
                    if cf.name == "settings.json" and info.is_valid:
                        try:
                            settings_data = json.loads(cf.read_text(encoding='utf-8'))
            ...
            if vscode_extensions and vscode_extensions.exists():
                self._scan_vscode_global_extensions(result, vscode_extensions, agent_id)
    ...
    def _scan_vscode_global_extensions(self, result: AgentConfigScan, ext_path: Path, agent_id: str):
                ...
                if any(kw in item_lower for kw in keywords):
                    ...
                    if pkg_file.exists():
                        try:
                            ...
                            result.external_plugins.append(PluginInfo(
                                name=pkg.get("displayName", pkg.get("name", item.name)),
                                version=pkg.get("version"),
                                type=PluginType.EXTERNAL,
                                enabled=True,
                                source=str(item)
    ...
    def _scan_global_storage(self, result: AgentConfigScan, storage_path: Path):
        """扫描全局存储中的配置"""
        try:
        ...
        for agent_id, result in self.scan_results.items():
            ...
            if result.config_files:
                ...
                for cf in result.config_files:
                    ...
                    lines.append(f"- {status} `{cf.filename}` ({cf.size_bytes} bytes)\n")
                lines.append("\n")
            ...
            if result.external_plugins:
                ...
                for plugin in result.external_plugins[:10]:  # 最多显示10个
                    ...
                    lines.append(f"- {enabled} **{plugin.name}** v{plugin.version or '?'}\n")
                if len(result.external_plugins) > 10:
                    lines.append(f"- ... 还有 {len(result.external_plugins) - 10} 个插件\n")
                lines.append("\n")
            
            # MCP服务器
            if result.mcp_servers:
                lines.append(f"### MCP服务器 ({len(result.mcp_servers)}个)\n\n")
                for server in result.mcp_servers:
                    enabled = "✅" if server.enabled else "❌"
                    lines.append(f"- {enabled} **{server.name}** ({server.type})\n")
                lines.append("\n")
