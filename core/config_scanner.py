...
class ConfigScanner:
    ...
    def scan_claude_config(self, agent_path: Path) -> AgentConfigScan:
        ...
        try:
            ...
            # Claude Code 使用 claude.json 存储配置
            claude_json = agent_path / "claude.json"
            if claude_json.exists():
                info = self.parse_config_file(claude_json)
                result.config_files.append(info)
                
                if info.is_valid and info.content_preview:
                    # 提取技能/功能
                    skills_data = info.content_preview.get('skills', {})
                    for skill_name, enabled in skills_data.items():
                        result.skills.append(SkillInfo(
                            name=skill_name,
                            enabled=enabled,
