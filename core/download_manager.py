...
class DownloadManager:
    ...
    def get_download_links(self, item_ids: List[str] = None) -> List[Dict]:
        ...
        with self._lock:
            items = (
                self.items.values()
                if item_ids is None
                else [self.items[i] for i in item_ids if i in self.items]
            )
            for item in items:
                links.append(
                    {
    ...
    def get_all_links_markdown(self, item_ids: List[str] = None) -> str:
        ...
        for link in links:
            lines.append(
                f"| {link['name']} | `{link['url']}` | {link['size']} |"
            )
    ...
    def _download_file(self, item: DownloadItem) -> DownloadResult:
        ...
        try:
            ...
            req = urllib.request.Request(
                item.url,
                headers={"User-Agent": "Mozilla/5.0 (AI Agent Repair Tool)"},
            )
    ...
    def get_status(self, item_id: str = None) -> Dict:
        ...
        with self._lock:
            if item_id:
                ...
                return {
                    k: self._item_to_dict(v) for k, v in self.items.items()
                }
