...
class ErrorPatternDatabase:
    ...
    def find_matching_patterns(
        self, log_content: str, file_path: Optional[str] = None
        ...
        for pattern in self.patterns:
            ...
            if file_path and pattern.affected_files:
                file_matches = any(
                    fnmatch.fnmatch(file_path, af)
                    or fnmatch.fnmatch(Path(file_path).name, af)
...
class LogAnalyzer:
    ...
    def find_log_files(
        self, agent_path: Path, patterns: List[str]
    ) -> List[Path]:
        """查找日志文件"""
        log_files = []
    ...
    def analyze_agent(
        self, agent_path: Path, log_patterns: List[str]
        ...
        for log_file in log_files:
            ...
            # 查找匹配的错误模式
            matches = self.pattern_db.find_matching_patterns(
                log_content, relative_path
            )

            for pattern, confidence in matches:
                # 检查是否已存在相同问题的诊断结果
                existing = [
                    r for r in results if r.pattern_id == pattern.pattern_id
                ]
    ...
    def generate_report(self, results: List[DiagnosisResult]) -> str:
        ...
        for severity in [
            ...
            for result in by_severity[severity]:
                ...
                lines.append(
                    f"  影响文件: {', '.join(result.affected_files[:3])}"
                )
                if len(result.affected_files) > 3:
                    lines.append(
                        f"           等共{len(result.affected_files)}个文件"
                    )
                lines.append(
                    f"  自动修复: {'✓ 支持' if result.auto_fixable else '✗ 需手动'}"
                )
...
class AutoFixEngine:
    ...
    def apply_fix(
        self, diagnosis: DiagnosisResult, agent_path: Path
    ) -> Tuple[bool, str]:
        """
        应用自动修复
        返回: (是否成功, 消息)
    ...
    def _fix_replace_config(
        ...
        for file_path in diagnosis.affected_files:
            ...
            if full_path.exists():
                ...
                # 创建默认配置
                default_config = diagnosis.fix_params.get(
...
# 便捷函数
def diagnose_agent(
    agent_path: Path, log_patterns: List[str] = None
) -> List[DiagnosisResult]:
    """诊断Agent的便捷函数"""
