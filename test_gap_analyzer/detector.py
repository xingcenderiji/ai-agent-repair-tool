...
from .types import (
    CommitInfo, FileChange, CoverageGap, RiskLevel,
    HIGH_RISK_PATTERNS, MEDIUM_RISK_PATTERNS, SKIP_FILE_PATTERNS
)
...
class GapDetector:
    ...
    def _analyze_file_gaps(self, file_change: FileChange, commit: CommitInfo) -> List[CoverageGap]:
        """分析单个文件的覆盖缺口"""
        gaps = []
        ...
        try:
            ...
            for func in file_analysis.functions:
                if not self.code_analyzer.has_function_test(existing_tests, func.name):
                    risk_level = self._calculate_function_risk(func, file_analysis.content)
                    ...
                    gap = CoverageGap(
                        ...
                        reason=self._generate_reason(func, risk_level, commit.message),
                        code_snippet=self._extract_function_code(file_analysis.content, func),
                        affected_lines=list(range(func.start_line, func.end_line + 1))
                    )
                    gaps.append(gap)
    ...
    def _detect_pattern_gaps(self, file_path: str, content: str, 
                             file_change: FileChange, commit: CommitInfo) -> List[CoverageGap]:
        """检测代码中的风险模式"""
        gaps = []
    ...
    def _generate_reason(self, func, risk_level: RiskLevel, commit_msg: str) -> str:
        """生成缺口原因描述"""
        reasons = []
