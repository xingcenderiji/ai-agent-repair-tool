...
from .types import AnalyzerConfig, AnalysisResult, RiskLevel
...
class TestGapAnalyzer:
    ...
    def __init__(self, config: AnalyzerConfig):
        ...
        self.test_generator = TestGenerator(config.project_root, config.test_dir)
    ...
    def _generate_recommendations(self, result: AnalysisResult):
        """生成建议"""
        high_risk = [g for g in result.detected_gaps if g.risk_level == RiskLevel.HIGH]
        medium_risk = [g for g in result.detected_gaps if g.risk_level == RiskLevel.MEDIUM]
    ...
    def print_report(self, result: AnalysisResult):
        ...
        if result.detected_gaps:
            high_risk = [g for g in result.detected_gaps if g.risk_level == RiskLevel.HIGH]
            medium_risk = [g for g in result.detected_gaps if g.risk_level == RiskLevel.MEDIUM]
            low_risk = [g for g in result.detected_gaps if g.risk_level == RiskLevel.LOW]
...
示例:
  ...
  python -m test_gap_analyzer.analyze --write
    ...
    # 退出码
    has_high_risk = any(g.risk_level == RiskLevel.HIGH for g in result.detected_gaps)
