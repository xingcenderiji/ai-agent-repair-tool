...
class RepairAPIServer:
    ...
    def setup_routes(self):
        ...
        self.app.router.add_get(
            "/api/agents/{agent_id}/status", self.get_agent_status
        )
        self.app.router.add_post(
            "/api/agents/{agent_id}/scan", self.scan_agent
        )
        self.app.router.add_post(
            "/api/agents/{agent_id}/repair", self.repair_agent
        )
        self.app.router.add_post(
            "/api/agents/{agent_id}/backup", self.backup_agent
        )

        self.app.router.add_get("/api/backups", self.list_backups)
        self.app.router.add_post(
            "/api/backups/{backup_name}/restore", self.restore_backup
        )
        self.app.router.add_delete(
            "/api/backups/{backup_name}", self.delete_backup
        )
        ...
        self.app.router.add_static(
            "/", path=Path(__file__).parent, show_index=True
        )
            ...
            return web.json_response(
                {
                    ...
                    "data": {
                        ...
                        "config_path": (
                            str(info.config_path) if info.config_path else None
                        ),
                        "version": info.version,
    ...
    async def list_backups(self, request):
        ...
        try:
            ...
            return web.json_response({"success": True, "data": backups})
        except Exception as e:
            return web.json_response(
                {"success": False, "error": str(e)}, status=500
            )
    ...
    async def delete_backup(self, request):
        ...
        try:
            ...
            if backup_path.exists():
                ...
                return web.json_response(
                    {"success": True, "message": f"备份 {backup_name} 已删除"}
                )
            else:
                return web.json_response(
                    {"success": False, "error": "备份不存在"}, status=404
                )
        except Exception as e:
            return web.json_response(
                {"success": False, "error": str(e)}, status=500
    ...
    async def batch_backup(self, request):
        ...
        try:
            ...
            return web.json_response(
                {
                    "success": True,
                    "data": {"total": len(agent_ids), "results": results},
                }
            )
        except Exception as e:
            return web.json_response(
    ...
    async def get_repair_report(self, request):
        ...
        try:
            ...
            return web.json_response({"success": True, "data": report})
        except Exception as e:
            return web.json_response(
                {"success": False, "error": str(e)}, status=500
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AI Agent 修复工具 API 服务器"
    )
