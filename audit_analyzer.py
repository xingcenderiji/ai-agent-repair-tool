...
示例:
  ...
  %(prog)s --agent cursor --count 20 # 查看Cursor的最近20个操作
    ...
    parser.add_argument("--failed", action="store_true",
                        help="显示失败操作")
    parser.add_argument("--agent", dest="agent_id", metavar="ID",
                        help="显示指定Agent的操作历史")
    ...
    # 如果没有参数，显示帮助
    if not any([args.list, args.session_id, args.report, args.failed, args.agent_id]):
        parser.print_help()
        return
