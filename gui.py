#!/usr/bin/env python3
"""
Tiangong (天工) - AI Agent Repair Tool
...
class RepairHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif parsed.path.startswith("/api/read-file"):
            query = urllib.parse.parse_qs(parsed.query)
            file_path = query.get("path", [""])[0]
            if not file_path:
                self._json_response({"error": "缺少 path 参数"})
            else:
                try:
                    import pathlib
...
<div class="container">
  ...
  <div id="phase-preview" class="hidden">
    <div class="card">
      ...
      <div class="actions">
        ...
        <button class="btn btn-primary" onclick="showReview(currentState)">下一步: 审查 →</button>
      </div>
    </div>
  ...
  <div id="phase-confirm" class="hidden">
    ...
    <div class="actions">
      <button class="btn btn-secondary" onclick="showReview(currentState)">返回审查</button>
  ...
  installedAgents.forEach(agent => {
    ...
    if (scan.config_files && scan.config_files.length > 0) {
      ...
      scan.config_files.forEach(cf => {
        ...
        const safePath = (cf.path || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'");
        html += `<div class="config-item clickable" onclick="event.stopPropagation();showFileContent('${safePath}')">`;
  ...
  installedAgents.forEach(agent => {
    fetch(`/api/agent-links/${agent.agent_id}`)
      ...
      .then(links => {
        ...
        if (!container) return;
        let linkHtml = '';
        if (links.website) linkHtml += `<a href="${links.website}" target="_blank" class="agent-link-btn" title="官方网站" onclick="event.stopPropagation()">🌐 官网</a>`;
        if (links.docs) linkHtml += `<a href="${links.docs}" target="_blank" class="agent-link-btn" title="官方文档" onclick="event.stopPropagation()">📖 文档</a>`;
        if (links.github) linkHtml += `<a href="${links.github}" target="_blank" class="agent-link-btn" title="GitHub 仓库" onclick="event.stopPropagation()">💻 GitHub</a>`;
        if (links.issues) linkHtml += `<a href="${links.issues}" target="_blank" class="agent-link-btn" title="已知问题" onclick="event.stopPropagation()">🐛 问题</a>`;
        if (links.feedback) linkHtml += `<a href="${links.feedback}" target="_blank" class="agent-link-btn" title="意见反馈" onclick="event.stopPropagation()">💬 反馈</a>`;
    ...
    modal.innerHTML = `
      <div class="file-content-box">
        <div class="file-content-header">
          <span>📄 ${data.path}</span>
          <button class="file-content-close" onclick="this.closest('.file-content-modal').remove()">&times;</button>
        </div>
        <div class="file-content-body">
          <pre>${data.content.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
        </div>
      </div>
    `;
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    const modal = document.querySelector('.file-content-modal');
